...
local function InitializeBypass()
    pcall(function()
        ...
        if CMode then
            ...
            CMode.GetContentDiffData = function() return true, "CLEAN" end
        end

        -- ============ 3. TSS SDK COMPLETE KILL ============
        ...
        if TssSdk then
            ...
            TssSdk.GetFileMD5 = function() return "00000000000000000000000000000000" end
            ...
            TssSdk.GetVerdict = function() return "CLEAN" end
            TssSdk.IsClientClean = retTrue
        end

        -- ============ 4. HIGGSBOSON COMPLETE KILL ============
        ...
        if Higgs then
            ...
            if Higgs.StaticShowSecurityAlertInDev then Higgs.StaticShowSecurityAlertInDev = nop end
        ...
        if slua.isValid(pc0) then
            if pc0.HiggsBoson then
                pc0.HiggsBoson.bMHActive = false
                pc0.HiggsBoson.bCallPreReplication = false
            end
            if pc0.HiggsBosonComponent then
                pc0.HiggsBosonComponent.bMHActive = false
                pc0.HiggsBosonComponent.bCallPreReplication = false
                if pc0.HiggsBosonComponent.ControlMHActive then
                    pcall(function() pc0.HiggsBosonComponent:ControlMHActive(0) end)
                end
            end
        end

        -- ============ 5. ACE / XIGNCODE / BATTLEYE ============
        ...
        if ace then
            ...
            ace.IsClean = retTrue
            ace.GetVerdict = function() return "CLEAN" end
        end
        ...
        for _, path in ipairs(reportPaths) do
            ...
            if not module then
                ...
                if ok then module = m end
            end
            if module then
                for _, fn in ipairs({"Report","SendReport","ReportEvent","ReportException","ReportData",
                                     "ReportTLogEvent","OnSyncBanInfo","OnVoiceBanNotify","ReqBanInfo",
                                     "AskForInspector","ReportEnemy","KickOutOneTeam","ReportSuspicious",
                                     "ReportCheat","ReportHack","GetCarrierInfo","CheckIfCanCreateRole",
                                     "_OnHawkSync","_OnHawkReportSuccess","OnHandleBehaviorScore",
                                     "AIPerceptionScore","ReportAllPlayerInfo","AddRecordMLAIInfo","ReportAI"}) do
                    if module[fn] then module[fn] = nop end
                end
            end
        end

        -- ============ 7. GAMEPLAY CALLBACKS ============
        if _G.GameplayCallbacks then
            local GC = _G.GameplayCallbacks
            for _, fn in ipairs({"ReportAttackFlow","ReportSecAttackFlow","ReportHurtFlow",
                                  ...
                                  "ReportHitFlow","ReportPlayersPing","ReportPlayerIP",
                                  "ReportPlayerFramePingRecord","ReportDSNetSaturation",
                                  "ReportNetContinuousSaturate","ReportDSNetRate","ReportCircleFlow",
                                  "ReportJumpFlow","ReportMatchRoomData","SendClientStats",
                                  "SendServerAvgTickDelta","ReportIDCardProduceFlow",
                                  "ReportIDCardPickUpFlow","ReportIDCardDestroyFlow","ReportRevivalFlow",
                                  "ReportGameSetting","ReportGameSettingNew","ReportCommonInfo",
                                  "ReportLightweightStat","SendSecTLog","SendDataMiningTLog",
                                  "SendActivityTLog","OnDSConnectionSaturated"}) do
                GC[fn] = nop
            end
        ...
        -- ============ 8. NETUTIL PACKET BLOCK ============
        if NetUtil and NetUtil.SendPacket then
            local orig = NetUtil.SendPacket
            local blocked = {
                ["ReportAttackFlow"]=1,["ReportSecAttackFlow"]=1,["ReportHurtFlow"]=1,
                ["ReportFireArms"]=1,["ReportVerifyInfoFlow"]=1,["ReportMrpcsFlow"]=1,
                ["ReportPlayerBehavior"]=1,["ReportTeammatHurt"]=1,["ReportAimFlow"]=1,
                ["ReportHitFlow"]=1,["log_shooting_miss"]=1,["ReportCircleFlow"]=1,
                ["ReportJumpFlow"]=1,["report_players_ping"]=1,["report_player_ip"]=1,
                ...
                ["report_modded_files"]=1,["on_tss_sdk_anti_data"]=1,
                ...
                ["ReportUnknown"]=1,["report_parachute_data"]=1,["report_wall_hack"]=1,
                ["report_speed_hack"]=1,["report_vehicle_exception"]=1,
                ["ReportVehicleMoveFlow"]=1,["ReportSecVehicleMoveFlow"]=1,
                ["ReportPlayerMoveRoute"]=1,["ReportSecTgameMovingFlow"]=1,
            }
        ...
        -- ============ 9. SHOOT VERIFY / FILE CHECK ============
        local SubMgr = require("GameLua.GameCore.Module.Subsystem.SubsystemMgr")
        if SubMgr then
            for _, name in ipairs({"ShootVerifySubSystemClient","FileCheckSubsystem",
                                    "AFKReportorSubsystem","AvatarExceptionSubsystem",
                                    "ClientDataStatistcsSubsystem"}) do
                local sub = SubMgr:Get(name)
                if sub then
                    for k, v in pairs(sub) do
                        if type(v) == "function" and (k:find("Report") or k:find("Verify")
                           or k:find("Check") or k:find("Send") or k:find("Upload")) then
...
local function InstallHooks()
    pcall(function()
        ...
        if EventSystem then
            ...
            if oldPost and not _G.__PAKxEventHooked then
                _G.__PAKxEventHooked = true
        ...
        -- ============ GAMEPLAY CALLBACKS (report events) ============
        local GC = _G.GameplayCallbacks
        if GC and GC.OnDSPlayerStateChanged then
            ...
            GC.OnDSPlayerStateChanged = function(UID, state, ...)
                local blocked = { cheatdetected = true, violationdetected = true,
                                   suspiciousdetected = true, integrityfailure = true,
                                   securityviolation = true, banned = true, kicked = true }
        ...
        if Client and Client.GEMReportSubEvent then
            ...
            Client.GEMReportSubEvent = function(hud, eventName, ...)
                local blocked = { PufferEvent = true, GRomeLinkEvent = true,
                                   NetProxyEvent = true, AntiCheatEvent = true,
                                   SecurityReport = true }
        ...
        if Client and Client.GEMReportEnterLobbyEvent then
...
if not isExpired then
    ...
    Notify("VIP Mod loaded - Simplified Bypass + Aimbot V2")
