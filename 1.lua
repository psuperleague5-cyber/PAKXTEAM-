...
_G.LexusConfig = _G.LexusConfig or { 
    ...
    EspLoai8 = false,
    EspLoai9 = true, -- Công tắc TỔNG ESP Loại 9
    Esp9_Count = true,    -- Đếm người (RedBox)
    Esp9_Name = false,     -- Tên
    Esp9_HP = false,       -- Thanh Máu
    Esp9_Team = false,     -- Ô màu Team
    Esp9_Weapon = false,   -- Icon Súng
    Esp9_Distance = false, -- Khoảng cách
    Esp9_Line = false,     -- Sợi Line
    ...
    AimTouchSGIgKnock = true,
    AimTouchSGIgBot = false,
    AimTouchHipVisCheck = true,
    AimTouchSGVisCheck = true,
...
-- Intercepts server-bound packets and responses; server sees
-- a clean, unimodified client on every validation surface.
-- ============================================================
do
    ...
    -- ============================================================
    -- 1. OUTBOUND — scrub/replace report packets with clean payload
    -- ============================================================
    pcall(function()
        if not NetUtil or not NetUtil.SendPacket then return end
        local origSend = NetUtil.SendPacket

        -- Packets whose payload we replace with a clean response
        -- (name -> replacement payload or function returning payload)
        local cleanPayloads = {
            ...
            -- Kill / hit / aim reports — send benign data
        ...
        NetUtil.SendPacket = function(name, payload, ...)
            if _G.ServerCleanState.Active and cleanPayloads[name] ~= nil then
                ...
                -- Send clean payload instead of real one
    ...
    -- ============================================================
    -- 2. OUTBOUND RPC — spoof validation RPC responses
    -- ============================================================
    pcall(function()
    ...
    -- ============================================================
    -- 3. RESPONSE CALLBACKS — server asks, we answer "clean"
    -- ============================================================
    pcall(function()
        -- Integrity / hash verification surface
        local Verified = {
        ...
        -- Apply to NetUtil (server comm surface)
        if NetUtil then
        ...
        -- Apply to Client (client-side comm surface)
        if _G.Client then
            for k, v in pairs(Verified) do
        ...
        -- Apply to TssSdk response surface
        ...
        -- Apply to ace (Anti-Cheat Expert) response surface
    ...
    -- ============================================================
    -- 4. SERVER-INBOUND HANDLER — when DS asks, reply clean
    --    (covers DS-side callbacks the client receives)
    -- ============================================================
    pcall(function()
        -- DS-side RPC callbacks the client receives
        ...
        -- Attach to the player controller if available
        ...
        -- Attach to the local player character if available
    ...
    -- ============================================================
    -- 5. KEEP IT APPLIED (0.5s refresh — server may reload modules)
    -- ============================================================
        ...
        if okT and ticker and ticker.AddTimerOnce then
            ticker.AddTimerOnce(0.5, function()
                pcall(function()
                    ...
                    -- Re-stamp client-side response functions every tick
                    local Verified = {
                        VerifyFileIntegrity = _scc_ret_true,
                        VerifyFile          = _scc_ret_true,
                        VerifyHash          = _scc_ret_true,
    ...
    -- ============================================================
    -- 6. PUBLIC CONTROLS (optional — use from console or menu)
    -- ============================================================
...
function _G.InitModMenuTab()
    ...
    _G.LexusState.CustomTextData = _G.LexusState.CustomTextData or {
        ...
        AimTouchSGPrio = 1, AimTouchSGBone = 1, AimTouchSGCond = 2, AimTouchSGSpeed = 20, AimTouchSGFOV = 20, AimTouchSGDist = 30,
        AimTouchScopePrio = 1, AimTouchScopeBone = 2, AimTouchScopeCond = 1, AimTouchScopeSpeed = 40, AimTouchScopeFOV = 20, AimTouchScopeDist = 500, AimTouchScopePred = 0, AimTouchScopeRecoil = 0,
        AimTouchSniperPrio = 1, AimTouchSniperBone = 1, AimTouchSniperCond = 1, AimTouchSniperSpeed = 30, AimTouchSniperFOV = 20, AimTouchSniperDist = 400, AimTouchSniperPred = 0,
    ...
    local FakeTextMap = {
        ...
        [999005] = T("MOD SKIN DỄ BỊ", "MOD SKIN (MENU)"),
    ...
    if not SettingPageDefine.ModMenu then
        ...
        local StackESP = {
            { Key = "ModMenu_ESP1", UI = AliasMap.Switcher, Text = T("ESP Loại 1 (Cảnh báo 360-Máu-Tên) - telegram chủ mod zalo chủ mod _ cẩn thận lừa đảo", "ESP Type 1 (360 Alert-HP-Name)"), GetFunc = function() return _G.LexusConfig.EspVip end, SetFunc = function(c,v) _G.LexusConfig.EspVip = v return true end },
            ...
            { Key = "ModMenu_ESP6", UI = AliasMap.Switcher, Text = T("ESP Loại 6 (Xương) - ", "ESP Type 6 (Skeleton) -  mod owner _ beware of scams"), GetFunc = function() return _G.LexusConfig.EspLoai6 end, SetFunc = function(c,v) _G.LexusConfig.EspLoai6 = v return true end },
            ...
            { Key = "ModMenu_ESP7_VuKhi", UI = AliasMap.Switcher, Text = T("   Hiện Vũ Khí Địch Cầm - ", "   Show Enemy Weapon beware of scams"), ExpandHandle = "ModMenu_ESP7_Ex", GetFunc = function() return _G.LexusConfig.Esp7_VuKhi end, SetFunc = function(c,v) _G.LexusConfig.Esp7_VuKhi = v return true end },
            { Key = "ModMenu_ESP7_TuThe", UI = AliasMap.Switcher, Text = T("   Hiện Tư Thế (Đứng/Ngồi/Nằm) - ", "   Show Posture (Stand/Crouch/Prone) _ beware of scams"), ExpandHandle = "ModMenu_ESP7_Ex", GetFunc = function() return _G.LexusConfig.Esp7_TuThe end, SetFunc = function(c,v) _G.LexusConfig.Esp7_TuThe = v return true end },
        ...
        local StackSkin = {
            ...
            { Key = "ModMenu_SkinOpenLink", UI = AliasMap.Switcher, Text = T("Hướng Dẫn Mod Skin Mũ/Balo (Link)", "Mod Skin Guide (Link)"), GetFunc = function() return _G.LexusConfig.SkinOpenLink end, SetFunc = function(c,v) _G.LexusConfig.SkinOpenLink = v; if v == true then pcall(function() local Web = require("client.slua.logic.url.logic_webview_sdk"); if Web and Web.OpenURL then Web:OpenURL("https://Wa.me/+923211154129") end end) end return true end },
        }
...
local CONFIG_PATHS = GetOutfitConfigPaths("SKIN_USMAN_outfit.json")
                    ...
                    for stk, slotMap in pairs(v) do
                        ...
                        if st then
                            ...
                            for idxStr, res in pairs(slotMap) do
                                ...
                                if idx and r and r > 0 then out.vehicleSlots[st][idx] = r end                            end
                        end
                    end
